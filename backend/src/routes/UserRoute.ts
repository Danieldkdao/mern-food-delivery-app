...
userRoute.post('/register', async (req: Request<{}, {}, UserInfo>, res: Response) => {
    try {
        const { name, email, password } = req.body;
        const hashedPassword = await bcrypt.hash(password, 10);
        const response: CommonRes = await registerUser(name, email, hashedPassword);
        if(!response?.success) return res.json(response);
        const logged = await loginUser(email, password);
        if(!logged?.success || !logged.payload) return res.json(logged);
...
userRoute.post('/login', async (req: Request<{}, {}, Login>, res: Response) => {
    try {
...
userRoute.get('/dashboard', authenticateUser, (req: Request, res: Response) => {
    res.json({success: true, message: "User authorized successfully!", info: req.user});
});

userRoute.get('/refresh', async (req: Request, res: Response) => {
    const oldRefreshToken = req.cookies.refreshToken;
    if(!oldRefreshToken) return res.json("Token not found.");
    try {
        const {id, name} = jwt.verify(oldRefreshToken, process.env.REFRESH_TOKEN_SECRET!) as JwtPayload;
...
userRoute.delete('/logout', async (req: Request, res: Response) => {
    const token = req.cookies.refreshToken;
    if(!token) return res.json({success: false, message: "No token exists."});
...
userRoute.put('/addToCart', async (req: Request<{}, {}, UpdateCartAdd>, res: Response) => {
...
userRoute.get('/getCart/:userId', async (req: Request<ParamUser>, res: Response) => {
    const response = await getCart(req.params.userId);
    res.json(response);
});

userRoute.get('/getCartPopulate/:userId', async (req: Request<ParamUser>, res: Response) => {
    const response = await getCartPopulate(req.params.userId);
    res.json(response);
});

type UpdateCartRemove = {
    userId: string
    itemId: string
    zero: boolean
}

userRoute.put('/removeCart', async (req: Request<{}, {}, UpdateCartRemove>, res: Response) => {
...
userRoute.delete('/removeCartItem', async (req: Request<{}, {}, {}, CartQuery>, res: Response) => {
...
userRoute.put('/updateQuantity', async (req: Request<{}, {}, UpdateCartQ>, res: Response) => {
    const { userId, itemId, num } = req.body;
    const response = await updateQuantity(userId, itemId, num);
    res.json(response);
});

userRoute.post('/createOrder', async (req: Request<{}, {}, {userId: string, total: number}>, res: Response) => {
    const response = await createOrder(req.body.userId, req.body.total);
    res.json(response);
});

userRoute.get('/getOrders/:userId', async (req: Request<{userId: string}>, res: Response) => {
